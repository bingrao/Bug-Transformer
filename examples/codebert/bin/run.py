...
from onmt.utils.earlystopping import EarlyStopping, Scorer

from bleu import bleuFromMaps, computeMaps
...
class stats(object):
    def __init__(self, eval_ppl, global_step, train_loss):
        self._acc = train_loss
        self._ppl = eval_ppl
...
def read_examples(args, filename):
    ...
    if len(filename) == 1 and args.app == "code2nl":
        examples = load_jsonl_examples(filename[0])
    elif len(filename) == 2 and args.app == "bug2fix":
        examples = load_pair_src_examples(filename)
    else:
        raise ValueError(f"The app {args.app} and input files do not match: {filename}")
...
def load_pair_src_examples(filename):
    """Read examples from filename."""
    examples = []
    src_filename = filename[0]
    trg_filename = filename[1]
    idx = 0
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        if example_index < 5 and args.verbose:
            if stage == 'train':
                args.logger.info("*** Example ***")
                args.logger.info("idx: {}".format(example.idx))

                args.logger.info("source_tokens: {}".format([x.replace('\u0120', '_') for x in source_tokens]))
                args.logger.info("source_ids: {}".format(' '.join(map(str, source_ids))))
                args.logger.info("source_mask: {}".format(' '.join(map(str, source_mask))))

                args.logger.info("target_tokens: {}".format([x.replace('\u0120', '_') for x in target_tokens]))
                args.logger.info("target_ids: {}".format(' '.join(map(str, target_ids))))
                args.logger.info("target_mask: {}".format(' '.join(map(str, target_mask))))
...
def get_arguments(parser):
    ...
    group.add('--work_model', '-work_model', type=str, default='preprocess',
              choices=['preprocess', 'train', 'translate'],
              help="Indicator that what kinds of task is working on right now")

    group.add('--app', '-app', type=str, default='code2nl',
    ...
    group.add('--early_stopping', '-early_stopping', type=int, default=10,
...
def main(parser=None):
    ...
    if os.path.exists(args.checkpoints_dir) is False:
    ...
    if args.load_model_path is not None:
        args.logger.info("reload model from {}".format(args.load_model_path))
    ...
    if args.do_train:
        ...
        train_examples = read_examples(args, args.train_filename)
        ...
        earlystopper = EarlyStopping(args.early_stopping) if args.early_stopping > 0 else None
        ...
        args.logger.info("  Num examples = %d", len(train_examples))
        args.logger.info("  Batch size = %d", args.train_batch_size)
        args.logger.info("  Num epoch = %d",
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) % args.eval_steps == 0) and eval_flag:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = read_examples(args, args.dev_filename)
                ...
                args.logger.info("\n***** Running evaluation *****")
                args.logger.info("  Num examples = %d", len(eval_examples))
                ...
                if earlystopper is not None:
                    earlystopper(stats(**result), global_step)

                    if earlystopper.is_improving():
                        model_to_save = model.module if hasattr(model,
                                                                'module') else model  # Only save the model it-self
                        output_model_file = os.path.join(args.checkpoints_dir,
                                                         f'checkpoint_best_{global_step}_{result["eval_ppl"]:.4f}.pt')
                        torch.save(model_to_save.state_dict(), output_model_file)
                        args.logger.info(f"Save best model to {output_model_file}")
                else:
                    ...
                    if eval_loss < best_loss:
                        args.logger.info("  Best ppl:%s", round(np.exp(eval_loss), 5))
                        args.logger.info("  " + "*" * 20)
                        best_loss = eval_loss
                ...
                (goldMap, predictionMap) = computeMaps(predictions, os.path.join(args.output_dir, "dev.gold"))
                dev_bleu = round(bleuFromMaps(goldMap, predictionMap)[0], 2)
                args.logger.info("  %s = %s " % ("bleu-4", str(dev_bleu)))
                args.logger.info("  " + "*" * 20)
                if dev_bleu > best_bleu:
                    args.logger.info("  Best bleu:%s", dev_bleu)
                    args.logger.info("  " + "*" * 20)
                    best_bleu = dev_bleu
